...
- `support-tickets`: Streamlit support service.
...
- `chatbot`: staged as an always-on Streamlit service and enabled only when its container CI is merged and a server-side `OPENAI_API_KEY` is configured.
...
Runtime secrets belong in GitHub Environment/Actions secrets or a host secret manager. Never commit them. `OPENAI_API_KEY` is only consumed if the chatbot is explicitly enabled.

## Public website integration

Do not change the website badge to `ONLINE` based on repository state. Public status should be connected only after a protected ingress exists (for example Cloudflare Tunnel/reverse proxy) and the browser can fetch the control-plane status endpoint without exposing administrative endpoints. Until then, the static website and the private runtime are intentionally separated.
